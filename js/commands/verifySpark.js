...
const otherPerson = async (interaction) => {
    ...
    const emails = await promises_1.default.readFile("json/verified.json", "utf-8").then(JSON.parse);
 ...
 * @returns bool Whether or not there is another user with the same NetID
 */
const isRegistered = async (interaction) => {
    let netid = interaction.options.getString("netid");
    const email = netid + "@uic.edu";
    const emails = await promises_1.default.readFile("json/emails.json", "utf-8").then(JSON.parse);
    if (!emails.includes(email)) {
        await interaction.reply({
            embeds: [{
                    author: {
