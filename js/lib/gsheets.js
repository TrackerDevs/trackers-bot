...
const VGSHEET_ID = "1MEPdqLt4XTTFoaVuL4nqrJ-uL5IDlsvcoM58d7UkD3s";
...
async function fetchEmails(auth) {
    ...
    const res = await sheets.spreadsheets.values.get({
        spreadsheetId: VGSHEET_ID,
        range: 'Sheet1!C2:C',
    });
    await fs.writeFile('json/emails.json', JSON.stringify(res.data.values.flat().map(email => email.toLowerCase())));
    return auth;
}
async function fetchVerified(auth) {
    ...
    await fs.writeFile('json/verified.json', JSON.stringify(((_a = res.data.values) === null || _a === void 0 ? void 0 : _a.flat()) || []));
    return auth;
}
