...
exports.parsePDFSchedule = void 0;
...
const getHeadingIndexes = (rows, search) => {
    ...
    for (i = 0; i < rows.length; i++)
        if (search.every((item) => rows[i].includes(item)))
            indexes.push(i);
    return indexes;
};
...
const parseClass = (classRow) => {
    ...
    const vagueTime = luxon_1.DateTime.fromJSDate(startDate, { zone: "America/Chicago" });
...
const parsePDFSchedule = async (pdfURL) => {
    try {
...
exports.parsePDFSchedule = parsePDFSchedule;
